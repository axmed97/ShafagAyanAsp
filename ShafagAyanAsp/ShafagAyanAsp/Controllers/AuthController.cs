...
namespace ShafagAyanAsp.Controllers
{
    public class AuthController : Controller
    {
        ...
        public async Task<IActionResult> ForgotPassword(ForgotPassword forgotPassword)
        {
            ...
            AppUser appUser = await _userManager.FindByEmailAsync(forgotPassword.Email);
            if (appUser == null)
            {
            ...
            var link = Url.Action("ResetPassword", "Auth", new { email = forgotPassword.Email, token = token }, Request.Scheme);

            var emailInfo = _context.EmailInfos.First();

            MailMessage mailMessage = new MailMessage();
            mailMessage.From = new MailAddress(emailInfo.Email, "Ayan Shafag");
            mailMessage.To.Add(new MailAddress(forgotPassword.Email));
        ...
        public async Task<IActionResult> ResetPassword(ResetPasswordVM resetPasswordVM)
        {
            ...
            var result = await _userManager.ResetPasswordAsync(appUser, resetPasswordVM.Token, resetPasswordVM.Passwrod);
            if (result.Succeeded)
            {
